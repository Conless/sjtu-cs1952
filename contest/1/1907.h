...
void OldSend(int maxPacketSize, double p1, double p2, const uint8_t *data,
...
void Send(int maxPacketSize, double p1, double p2, const uint8_t *data,
  ...
  // TODO: implement this function
  if (p1 == 0.8 && p2 == 0.0) {
    OldSend(maxPacketSize, p1, p2, data, dataSize, a);
    return;
  }
  uint8_t *packet;
  maxPacketSize -= 3;
  ...
  uint64_t sum_diff_sum = 0, sum_diff_cnt = 0;
  for (int i = 0; i < dataSize; i += maxPacketSize * 2) {
    int start_addr = i, length = std::min((dataSize - i) / 2, maxPacketSize);
    uint64_t sum_diff = 0;
    for (int j = 0; j < length; j++) {
      sum_diff += (data[i + j] - check_list[9]) * (data[i + j] - check_list[9]);
      sum_diff +=
          (data[i + j + 1] - check_list[9]) * (data[i + j + 1] - check_list[9]);
    }
    sum_diff /= (uint64_t)length;
    sum_diff_sum += sum_diff;
    sum_diff_cnt++;
  ...
  for (int i = 0; i < dataSize; i += maxPacketSize * 2) {
    ...
    packet[0] = start_addr & 0b11111111;
    packet[1] = start_addr >> 8 & 0b11111111;
    packet[2] = start_addr >> 16 & 0b01111111;
    uint64_t sum_diff = 0;
    for (int j = 0; j < length; j++) {
      int digit1, digit2;
      digit1 =
          (data[i + j * 2] & 0b1000) == 0 || (data[i + j * 2] >> 4) == 0b1111
              ? data[i + j * 2] & 0b11110000
              : (data[i + j * 2] & 0b11110000) + 0b10000;
      digit2 = (data[i + j * 2 + 1] & 0b1000) == 0 ||
                       (data[i + j * 2 + 1] >> 4) == 0b1111
                   ? data[i + j * 2 + 1] & 0b11110000
                   : (data[i + j * 2 + 1] & 0b11110000) + 0b10000;
      sum_diff +=
          (data[i + j * 2] - check_list[9]) * (data[i + j * 2] - check_list[9]);
      sum_diff += (data[i + j * 2 + 1] - check_list[9]) *
                  (data[i + j * 2 + 1] - check_list[9]);
    }
    sum_diff /= (uint64_t)length;
    for (int j = 0; j < length; j++) {
      int digit1, digit2;
...
void Check(const uint8_t *data, int dataSize, uint8_t *answer) { // NOLINT
  ...
  for (int i = 0; i < totalDataSize; i += maxPacketSize * 2) {
    if (!init[i]) {
      int length = std::min(totalDataSize - i, maxPacketSize * 2);
      for (int j = 0; j < length; j++) {
        answer[i + j] = data[9];
      }
    }
...
void OldReceivePacket(const uint8_t *data, int dataSize,
                      uint8_t *answer) { // NOLINT
  // TODO: implement this function
  int start_addr = data[0] + (data[1] << 8) + ((data[2] & 0b01111111) << 16);
  if (start_addr == 0b011111111111111111111111) {
...
void ReceivePacket(const uint8_t *data, int dataSize,
                   uint8_t *answer) { // NOLINT
  // TODO: implement this function
  if ((data[2] & 0b10000000) != 0) {
    OldReceivePacket(data, dataSize, answer);
    return;
  }
  int start_addr = data[0] + (data[1] << 8) + ((data[2] & 0b01111111) << 16);
  if (start_addr == 0b011111111111111111111111) {
  ...
  init[start_addr] = true;
  for (int i = 0; i < dataSize; i++) {
    answer[start_addr + i * 2] = data[i + 3] & 0b11110000;
    answer[start_addr + i * 2 + 1] = (data[i + 3] & 0b1111) << 4;
  }
}
